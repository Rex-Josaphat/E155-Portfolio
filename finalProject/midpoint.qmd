...
All of these modules have been tested with respective testbenches to validate functionality and are combined into the **receiver()** top module.
...
![Key Generation by TRNG](docs/midpoint/keys.png){#fig-keyGen width=50% .lightbox}

- **main.c:** This file sets up our clock and peripherals and calls upon the functions in webpage.c to execute the webpage. 

Below is a sample SPI transaction from the MCU to the FPGA once the MCU receives the user message from ESP8226 via USART:

![Full SPI Transaction of Plaintext and Key](docs/midpoint/trace1.png){#fig-trace1 .lightbox}

![Logic Analyzer Trace of First Few Characters of Plaintext](docs/midpoint/trace2.png){#fig-trace2 .lightbox}

![Logic Analyzer Confirming SPI Transaction of The TRNG Generated Key](docs/midpoint/trace3.png){#fig-trace3 .lightbox}
...
![Receiver Module Block Diagram](docs/midpoint/receiverWeb.png){#fig-senderWeb .lightbox}
...
- **msgReceive.c:** This file reads 8 bytes based on states from the limit switches, and concatenates the full 16byte message sent in the end. It uses a hardware Interrupt configured where the pin receiving the transfer clock implements a rising trigger 
...
![Schemactic of the Sender Actuation Side](docs/midpoint/actuationSender.png){#fig-actSender .lightbox}

![Schemactic of the Receiver Actuation Side](docs/midpoint/actuationReceiver.png){#fig-actReceiver .lightbox}

The assembly of the LCD required a level shifter to boost voltages into the required range as shown below:

![Level Shifter and ESP8266 Schematic](docs/midpoint/levelShifter.png){#fig-shifter .lightbox}

## High Level Block Diagram

![High Level Block Diagram](docs/midpoint/blockDiagram.png){#fig-sysDiagram .lightbox}

## High Level Data Flow Diagram

![High Level Data Flow Diagram](docs/midpoint/data.png){#fig-message .lightbox}
